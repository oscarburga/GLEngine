...
struct SPbrMaterial {
	...
	int NormalTexIndex;
	int OcclusionTexIndex; // 52
	bool bIgnoreLighting; // 56
	int _pad1; // 60
	int _pad2; // 64
	// sampler2D ColorTex; // 72
	// sampler2D MetalRoughTex; // 80
	// sampler2D NormalTex; // 88
	// sampler2D OcclusionTex; // 96
};
...
void InitPbr(vec4 srcColor) 
{
	...
	if (pbrMaterial.NormalTexIndex >= 0) {
		N = texture(TexSamplers[pbrMaterial.NormalTexIndex], fs.TexCoords).rgb;
		N = (N * 2.f) - 1.f;
	...
	if (pbrMaterial.MetalRoughTexIndex >= 0) {
		vec4 metalRough = texture(TexSamplers[pbrMaterial.MetalRoughTexIndex], fs.TexCoords);
		PbrMetalness *= metalRough.b;
		PbrRoughness *= metalRough.g;
	}
	PbrAmbientOcclusion = 1.f; // temp
	if (pbrMaterial.OcclusionTexIndex >= 0) {
		PbrAmbientOcclusion = pbrMaterial.OcclusionStrength * texture(TexSamplers[pbrMaterial.OcclusionTexIndex], fs.TexCoords).r;
	}
